...
export const BuyerProductCard: React.FC<BuyerProductCardProps> = ({
  ...
  return (
    <div
      ...
      className="group bg-white rounded-2xl border border-kk-line/90 shadow-2xs interactive-hover-card kk-card-interactive overflow-hidden flex flex-col justify-between text-left cursor-pointer focus:outline-hidden focus:ring-2 focus:ring-kk-primary/40 h-full w-full max-w-full box-border select-none"
    >
      {/* Product Image Stage */}
      <div className="relative aspect-square w-full shrink-0 overflow-hidden bg-stone-100">
        <img
          ...
          alt={title}
          loading="lazy"
          className="w-full h-full object-cover object-center kk-card-img"
        />
        ...
        <button
          ...
          className={`absolute top-2 right-2 sm:top-2.5 sm:right-2.5 w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center shadow-xs backdrop-blur-xs z-10 interactive-hover-nav kk-icon-btn-interactive ${
            isWishlisted
      ...
      <div className="p-2.5 sm:p-3 flex flex-col flex-1 justify-between gap-1.5 min-w-0 w-full">
        ...
        <div className="pt-2 border-t border-stone-100 flex items-center justify-between gap-1 mt-auto w-full min-w-0">
          ...
          <span className="text-[10px] sm:text-[11px] font-semibold text-kk-primary bg-kk-primary-soft group-hover:bg-kk-primary group-hover:text-white px-2 sm:px-2.5 py-1 rounded-lg shrink-0 whitespace-nowrap shadow-2xs interactive-hover-pill kk-pill-interactive">
