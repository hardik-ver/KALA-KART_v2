...
import { buildArtisanProductInput } from "../../utils/pricingFeatureBuilder";
import { defaultArtisanProductRepo } from "../../services/pricing/artisanProductStore";
import {
  CheckCircle2,
  Share2,
  Sparkles,
...
export const MarketplaceListingScreen: React.FC<MarketplaceListingScreenProps> = ({
  ...
  const handlePublish = () => {
    ...
    if (catalog.artisanCosts && catalog.productionTimeHours) {
      const bridgeResult = buildArtisanProductInput(
        catalog,
        catalog.artisanCosts,
        activePrice
      );
      if (bridgeResult.success && bridgeResult.productInput) {
        defaultArtisanProductRepo.save(bridgeResult.productInput).catch((err) => {
          console.error("Failed to persist verified artisan product:", err);
        });
      }
    }
  ...
  return (
    <div className="flex flex-col h-full bg-kk-surface text-kk-ink p-4 sm:p-5 overflow-y-auto font-sans">
      ...
      <div className="mt-3 bg-white rounded-2xl border border-kk-line shadow-xs overflow-hidden flex flex-col">
        ...
        <div className="relative w-full h-52 sm:h-60 bg-stone-900 overflow-hidden flex items-center justify-center">
          <img
            ...
            alt={catalog.titleEn}
            className="w-full h-full object-contain bg-kk-surface"
          />

          {/* Top Badges */}
          <div className="absolute top-3 left-3 flex items-center gap-1.5">
            <span className="bg-kk-ink/90 text-white text-[11px] font-bold px-3 py-1 rounded-full backdrop-blur-xs flex items-center gap-1 border border-white/20">
          ...
          <div className="absolute bottom-3 right-3 bg-kk-primary text-white px-3.5 py-1.5 rounded-xl font-black text-lg shadow-lg flex items-center gap-1 border-2 border-white">
        ...
        <div className="p-4 space-y-3">
          ...
          <div>
            ...
            <h3 className="font-extrabold text-base sm:text-lg text-kk-ink">
              {catalog.titleHi}
            </h3>
          </div>
          ...
          <div className="p-3 bg-stone-50 rounded-xl border border-kk-line">
            <div className="flex items-center justify-between gap-2 mb-1">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="flex justify-between items-center p-2.5 bg-stone-50 rounded-xl border border-kk-line text-xs">
              <span className="text-stone-500 font-bold">वजन / Wt:</span>
              <span className="font-bold text-kk-ink">{catalog.specs.weight}</span>
            </div>
            <div className="flex justify-between items-center p-2.5 bg-stone-50 rounded-xl border border-kk-line text-xs">
              <span className="text-stone-500 font-bold">आकार / Dim:</span>
              <span className="font-bold text-kk-ink">{catalog.specs.dimensions}</span>
            </div>
          </div>
      ...
      <div className="mt-3.5 pt-3 border-t border-kk-line">
        {!isPublished ? (
          <button
            ...
            className="w-full py-3.5 rounded-xl bg-kk-primary hover:bg-kk-primary-dark text-white font-bold text-sm sm:text-base flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-transform"
          >
            {isPublishing ? (
              <>
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => setShowShareModal(true)}
                className="flex-1 py-3 bg-kk-ink hover:bg-kk-ink-soft text-white rounded-xl font-bold text-xs sm:text-sm flex items-center justify-center gap-2 shadow-sm"
              >
              ...
              <button
                type="button"
                onClick={onReset}
                className="flex-1 py-3 bg-white border border-kk-line hover:bg-stone-50 text-stone-700 rounded-xl font-bold text-xs sm:text-sm flex items-center justify-center gap-2 shadow-xs"
              >
      ...
      {showShareModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-sm w-full p-5 text-center shadow-2xl space-y-4">
            ...
            <div>
              <h3 className="font-extrabold text-lg text-kk-ink">
            ...
            <div className="p-4 bg-stone-50 rounded-2xl border border-kk-line inline-block mx-auto">
              <img
