...
export const VoiceCatalogScreen: React.FC<VoiceCatalogScreenProps> = ({
  ...
  return (
    <div className="flex flex-col min-h-0 h-full bg-kk-surface text-kk-ink p-3 sm:p-4 md:p-5 pb-6 sm:pb-8 overflow-y-auto font-sans">
      {/* Header Bar */}
      <div className="flex items-center justify-between gap-2 pb-2.5 sm:pb-3 border-b border-kk-line shrink-0">
      ...
      {/* Voice Input Section */}
      <div className="mt-2.5 sm:mt-3 bg-white border border-kk-line rounded-2xl p-4 sm:p-5 flex flex-col items-center text-center relative shadow-xs shrink-0">
        {isRecording && (
          <div className="absolute inset-0 bg-kk-primary/10 animate-pulse pointer-events-none rounded-2xl" />
        )}
        ...
        <div className="flex items-center justify-center gap-2 mb-1.5 sm:mb-2 px-2 text-center shrink-0">
          <span
            className={`inline-block w-2.5 h-2.5 rounded-full shrink-0 ${
              isRecording ? "bg-red-500 animate-ping" : isComplete ? "bg-emerald-500" : "bg-kk-primary"
            }`}
          />
          <span className="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-kk-ink leading-tight">
            {isRecording
              ? language === "hi"
        ...
        <div className="flex items-center justify-center gap-1.5 h-6 sm:h-7 my-1.5 sm:my-2 shrink-0">
          {audioWaveLevel.map((lvl, idx) => (
            <div
              key={idx}
              className="w-1.5 bg-kk-primary rounded-full transition-all duration-100"
              style={{
          ...
          <button
            ...
            className={`w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center transition-all duration-200 active:scale-95 shadow-xl border-4 select-none touch-manipulation flex-shrink-0 ${
              isRecording
                ...
                : "bg-kk-ink hover:bg-kk-ink-soft border-white text-white shadow-slate-900/30"
            }`}
          >
            {isRecording ? (
              <Mic className="w-8 h-8 text-white" />
            ) : (
              <Mic className="w-8 h-8 text-kk-primary" />
            )}
          </button>
        </div>
      ...
      {isAiParsing && (
        <div className="mt-3 p-4 bg-orange-50 border border-orange-200 rounded-2xl flex items-center gap-3 animate-pulse shrink-0">
          <div className="w-6 h-6 border-2 border-kk-primary border-t-transparent rounded-full animate-spin flex-shrink-0" />
      ...
      {conversationTurns.length > 0 && !isComplete && (
        <div className="mt-3 p-3 bg-white border border-kk-line rounded-2xl shadow-xs shrink-0">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-bold text-stone-700">
              {language === "hi" ? "सत्यापित विवरण प्रगति:" : "Verified Information Progress:"}
            </span>
            <span className="text-xs font-black text-kk-primary">
              {verifiedFactsCount} / 9 {language === "hi" ? "पूरे" : "Complete"}
            </span>
          </div>
          <div className="w-full bg-stone-100 rounded-full h-2 overflow-hidden">
            <div
              className="bg-kk-primary h-2 rounded-full transition-all duration-300"
        ...
        <div className="mt-3 bg-amber-50/70 border-2 border-amber-300 rounded-2xl p-4 shadow-sm shrink-0">
          ...
          <div className="flex items-start justify-between gap-2 pb-2.5 border-b border-amber-200">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-sm font-extrabold text-kk-ink">
          ...
          {(followUpPromptEn || followUpPromptHi) && (
            <div className="mt-2.5 p-3 bg-white rounded-xl border border-amber-200 shadow-xs">
              <div className="flex items-center gap-1.5 text-xs font-bold text-amber-800 mb-1">
                <Bot className="w-3.5 h-3.5 text-kk-primary" />
          ...
          <div className="mt-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {missingFields.map((item) => (
                <div
                  key={item.field}
                  className="flex items-center gap-2 p-2 bg-white rounded-lg border border-amber-200 text-xs font-semibold text-kk-ink"
                >
          ...
          <div className="mt-3 pt-2 flex items-center justify-center">
            <button
              ...
              onTouchEnd={stopRecording}
              disabled={isAiParsing}
              className="px-4 py-2.5 bg-kk-primary hover:bg-kk-primary-dark text-white rounded-xl font-bold text-xs flex items-center gap-2 shadow-md active:scale-95 transition-transform"
            >
      ...
      {conversationTurns.length > 0 && (
        <div className="mt-3 bg-white border border-kk-line rounded-2xl p-4 shadow-xs shrink-0">
          <div className="flex items-center justify-between mb-2 pb-2 border-b border-stone-100">
            <div className="flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-emerald-600" />
              <span className="text-xs font-extrabold text-kk-ink uppercase tracking-wider">
        ...
        <div className="mt-4 space-y-3 shrink-0">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center bg-stone-200 p-1 rounded-xl">
              <button
                ...
                className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${
                  reviewLanguage === "en"
                    ? "bg-white text-kk-ink shadow-xs"
                    : "text-stone-600"
                }`}
              >
                🌐 English Listing
            ...
            <button
              ...
              className="flex items-center gap-1 text-xs font-bold text-kk-primary hover:underline"
            >
          ...
          <div className="bg-white rounded-2xl border border-kk-line shadow-xs p-4 space-y-3">
            {/* Title Section */}
            <div>
              ...
              {isEditingListing ? (
                <input
                  ...
                  className="w-full p-2.5 text-sm font-bold border border-stone-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-kk-primary"
                />
              ) : (
                <h3 className="text-base sm:text-lg font-extrabold text-kk-ink">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs bg-stone-100 text-stone-800 font-bold px-2.5 py-0.5 rounded-md border border-kk-line">
                {catalog.category || selectedCraft.nameEn}
              </span>
            </div>

            {/* AI-Written Description (Distinguished & Editable) */}
            <div className="p-3.5 bg-stone-50 rounded-xl border border-kk-line">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-1.5">
                  <Sparkles className="w-3.5 h-3.5 text-kk-primary" />
              ...
              {isEditingListing ? (
                <textarea
                  ...
                  className="w-full p-2.5 text-xs text-stone-800 border border-stone-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-kk-primary"
                />
              ) : (
            ...
            {isEditingListing && (
              <div className="flex justify-end">
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2 bg-kk-primary text-white rounded-xl text-xs font-bold shadow-md hover:bg-kk-primary-dark transition-colors"
                >
            ...
            <div className="pt-2 border-t border-stone-100">
              <div className="flex items-center gap-1.5 text-xs font-bold text-kk-ink mb-2">
                <Tag className="w-3.5 h-3.5 text-kk-primary" />
              ...
              <div className="flex flex-wrap gap-1.5">
                {catalog.searchTags.map((tag, i) => (
                  <span
                    key={i}
                    className="text-xs bg-stone-100 text-stone-700 font-semibold px-2.5 py-1 rounded-md border border-kk-line"
                  >
                    {tag}
                  </span>
                ))}
      ...
      {conversationTurns.length > 0 && (
        <div className="mt-3 bg-stone-100/70 rounded-2xl p-3 border border-kk-line shrink-0">
          <span className="text-[11px] font-bold text-stone-500 uppercase tracking-wider block mb-2">
          ...
          <div className="space-y-2">
            {conversationTurns.map((turn) => (
              <div
                key={turn.id}
                className="p-2.5 bg-white rounded-xl border border-kk-line text-xs shadow-2xs"
              >
                <div className="flex items-center justify-between text-stone-400 font-medium mb-1">
                  <span className="font-bold text-kk-primary">
      ...
      <div className="mt-4 pt-3 pb-2 border-t border-kk-line shrink-0">
        <button
          type="button"
          ...
          className={`w-full py-3.5 rounded-xl font-bold text-sm sm:text-base flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-transform ${
            catalog && isComplete && !isAiParsing
              ? "bg-kk-primary hover:bg-kk-primary-dark text-white cursor-pointer"
    ...
    <div
      className={`p-2 rounded-xl border text-xs flex flex-col justify-between transition-colors ${
        isPresent
          ...
          : "bg-stone-50 border-kk-line text-stone-400 border-dashed"
      }`}
    >
      <span className="text-[10px] uppercase font-bold text-stone-500">{label}</span>
      <span className={`font-extrabold mt-0.5 truncate ${isPresent ? "text-kk-ink" : "italic"}`}>
        {value || "Missing"}
      </span>
    </div>
  );
...
const SpecRow: React.FC<{ icon: any; label: string; value: string | undefined }> = ({
  ...
  <div className="flex items-center justify-between p-2.5 bg-stone-50 rounded-xl border border-kk-line">
    <div className="flex items-center gap-2 text-stone-600">
      <Icon className="w-3.5 h-3.5 text-kk-primary" />
      <span className="font-semibold">{label}</span>
    </div>
    <span className="font-extrabold text-kk-ink">{value || "—"}</span>
  </div>
);
