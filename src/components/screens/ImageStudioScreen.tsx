...
export const ImageStudioScreen: React.FC<ImageStudioScreenProps> = ({
  ...
  return (
    <div className="flex flex-col h-full bg-kk-surface text-kk-ink p-4 sm:p-5 overflow-y-auto font-sans">
          ...
          <div className="relative w-full h-full flex items-center justify-center bg-black">
            ...
            <div className="absolute bottom-4 inset-x-0 flex items-center justify-center gap-4 px-4 z-20">
              ...
              <button
                ...
                className="w-16 h-16 rounded-full bg-kk-primary hover:bg-kk-primary-dark border-4 border-white shadow-2xl flex items-center justify-center text-white active:scale-90 transition-transform text-2xl"
        ...
        <div className="mt-2.5 p-3 bg-amber-50 border border-amber-200 rounded-2xl text-xs text-amber-900 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5 shadow-2xs">
          ...
          <div className="flex items-center gap-2 self-end sm:self-auto flex-shrink-0">
            <button
              type="button"
              onClick={() => fileInputRef.current?.click()}
              className="px-3 py-1.5 bg-kk-primary hover:bg-kk-primary-dark text-white rounded-xl text-xs font-semibold flex items-center gap-1.5 shadow-xs transition-colors active:scale-95"
            >
              <Upload className="w-3.5 h-3.5" />
              <span>{language === "hi" ? "गैलरी से चुनें" : "Select from Gallery"}</span>
            </button>
      ...
      <div className="mt-3 flex items-center justify-center gap-6 py-1">
        ...
        <div className="flex flex-col items-center gap-1">
          <button
            ...
            onClick={startCamera}
            disabled={isCameraStarting}
            className="w-14 h-14 sm:w-16 sm:h-16 bg-kk-ink hover:bg-kk-ink-soft text-white rounded-full flex items-center justify-center text-2xl shadow-lg border-4 border-white active:scale-95 transition-transform disabled:opacity-50"
        ...
        <div className="flex flex-col items-center gap-1">
          <button
            type="button"
            onClick={() => fileInputRef.current?.click()}
            className="w-14 h-14 sm:w-16 sm:h-16 bg-kk-primary hover:bg-kk-primary-dark text-white rounded-full flex items-center justify-center text-2xl shadow-lg border-4 border-white active:scale-95 transition-transform"
      ...
      {hasPhoto && (
        <div className="mt-2 bg-white p-3 rounded-2xl border border-kk-line shadow-xs">
          <div className="flex items-center justify-between text-xs font-bold text-stone-600 mb-2">
            ...
            <button
              ...
              onClick={() => currentRawImage && runStudioPipeline(currentRawImage, currentPreset, true)}
              className="flex items-center gap-1 text-kk-primary font-semibold hover:underline disabled:opacity-50"
            >
          ...
          <div className="grid grid-cols-3 gap-2">
            <button
              ...
              className={`p-2 rounded-xl text-center text-xs font-semibold border transition-all disabled:opacity-50 ${
                currentPreset === "clean-studio"
                  ? "bg-kk-surface border-kk-primary text-kk-ink shadow-xs font-bold ring-1 ring-kk-primary"
                  : "bg-stone-50 border-kk-line text-stone-600 hover:bg-stone-100"
              }`}
            >
              ⚪ {language === "hi" ? "क्लीन स्टूडियो" : "Clean Studio"}
            </button>
        ...
        <button
          ...
          className={`w-full py-3.5 rounded-xl font-bold text-sm sm:text-base flex items-center justify-center gap-2 transition-all ${
            !hasPhoto || isProcessing
              ...
              : "bg-kk-primary hover:bg-kk-primary-dark text-white shadow-lg active:scale-95"
          }`}
        >
          <span>
            {language === "hi"
