...
export const PricingAssistantScreen: React.FC<PricingAssistantScreenProps> = ({
  ...
  const handleProceed = () => {
    ...
    const enteredCosts: ArtisanCostData = {
      rawMaterialCostINR: Number(rawMaterialCost) || 0,
      laborCostINR: Number(laborCost) || 0,
      packagingCostINR: Number(packagingCost) || 0,
      otherCostINR: Number(otherCost) || 0,
      quantity: catalog.specs?.quantity ? parseInt(catalog.specs.quantity, 10) || 1 : 1,
    };

    const parsedHours =
      catalog.productionTimeHours ??
      parseProductionTimeString(specs.productionTime) ??
      undefined;

    const updatedCatalog: CatalogItem = {
      ...catalog,
      pricing: updatedPricing,
      artisanCosts: enteredCosts,
      productionTimeHours: parsedHours,
    };
  ...
  return (
    <div className="flex flex-col h-full bg-kk-surface text-kk-ink p-4 sm:p-5 overflow-y-auto font-sans">
      ...
      <div className="mt-3 bg-white rounded-2xl border border-kk-line shadow-xs overflow-hidden">
        <button
          type="button"
        ...
        >
          <div className="flex items-center gap-2.5">
            <div className="p-1.5 bg-kk-primary-soft text-kk-primary rounded-lg">
              <Layers className="w-4 h-4" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-kk-ink uppercase tracking-wide">
      ...
      <div className="mt-3.5 bg-white rounded-2xl border border-stone-200 p-3.5 shadow-xs">
        ...
        <div className="grid grid-cols-2 gap-2.5">
          {/* Raw Material Cost */}
          <div className="bg-stone-50 p-2.5 rounded-xl border border-kk-line focus-within:border-kk-primary focus-within:bg-kk-surface transition-colors">
      ...
      <div className="mt-3.5">
        <button
          ...
          className={`w-full py-3 px-4 rounded-xl font-bold text-sm flex items-center justify-center gap-2 shadow-xs transition-all active:scale-[0.99] ${
            isLoading
              ? "bg-kk-primary-soft text-stone-600 cursor-wait"
              : "bg-kk-primary hover:bg-kk-primary-dark text-white"
          }`}
        >
          {isLoading ? (
            <>
      ...
      {predictionResult && (
        <div className="mt-3.5 space-y-3">
          ...
          {predictionResult.status === "MARKET_DATA_UNAVAILABLE" && (
            <div className="p-4 bg-orange-50/80 rounded-2xl border-2 border-orange-400 text-kk-ink shadow-xs">
      ...
      <div className="mt-3.5 bg-white rounded-2xl border-2 border-kk-line p-3.5 shadow-xs">
        <div className="flex items-center justify-between mb-1">
          <label className="text-xs font-bold uppercase tracking-wider text-stone-800 flex items-center gap-1.5">
            <IndianRupee className="w-4 h-4 text-kk-primary" />
            {language === "hi" ? "आपका अंतिम बिक्री मूल्य" : "Your Selling Price"}
          </label>
          <span className="text-[10px] font-bold text-kk-primary uppercase">
            Artisan Decision
          </span>
        </div>

        <div className="mt-2 flex items-center bg-stone-50 rounded-xl border border-kk-line px-3 py-2 focus-within:border-kk-primary focus-within:ring-2 focus-within:ring-kk-primary-soft transition-all">
      ...
      <div className="mt-4 pt-3 border-t border-kk-line">
        <button
          ...
          className="w-full py-3.5 rounded-xl bg-kk-primary hover:bg-kk-primary-dark text-white font-bold text-sm sm:text-base flex items-center justify-center gap-2 shadow-md active:scale-95 transition-transform"
        >
          <span>
            {language === "hi"
