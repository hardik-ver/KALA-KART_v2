...
// Colours (low -> high): #0E0602, #451A03, #92400E, #D49A6A
const PALETTE = [
  [0.055, 0.024, 0.008],
  [0.271, 0.102, 0.012],
  [0.573, 0.251, 0.055],
...
// 8 vec3 slots (24 floats), first 4 used
const FLATTENED_PALETTE = new Float32Array([
  0.055, 0.024, 0.008,
  0.271, 0.102, 0.012,
  0.573, 0.251, 0.055,
  0.831, 0.604, 0.416,
...
const FS_SOURCE = `#ifdef GL_FRAGMENT_PRECISION_HIGH
...
export const WavesBackground: React.FC<WavesBackgroundProps> = ({ className = "" }) => {
  ...
  useEffect(() => {
    ...
    if (!gl) {
      console.warn("WebGL not supported for WavesBackground");
      return;
    }
    ...
    const render = (now: number) => {
      ...
      // Uniform updates according to spec
      gl.uniform3fv(colorsLoc, FLATTENED_PALETTE);          // 8 vec3 slots, first 4 used
      gl.uniform4f(sceneLoc, canvas.width, canvas.height, elapsedSeconds * 0.67, 4.0);
      gl.uniform4f(shapeLoc, 1.92, 0.86, 0.32, 0.08);        // zoom, intensity, paramA, warp
      gl.uniform4f(surfaceLoc, 2.62, 1.0, 0.0, 1.0);         // detail, contrast, brightness, saturation
      gl.uniform4f(finishLoc, 0.0, 0.0, 0.0, 0.09);          // hue, vignette, blur, grain
      gl.uniform4f(transformLoc, 1.0, 1.94, 0.04, 0.0);      // seed, rotate, drift, OKLab toggle
      gl.uniform4f(spaceLoc, 0.0, 0.0, 0.0, 0.0);            // offset.xy, pointer.xy — cursor off, no tracking
      gl.uniform4f(cursorLoc, 0.0, 2.0, 0.65, 0.46);         // presence = 0 disables all cursor effects
    ...
    // Initial resize and start render loop
    resize();
    rafId = requestAnimationFrame(render);
