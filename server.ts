...
const currentFilename = typeof import.meta?.url === "string" ? fileURLToPath(import.meta.url) : (typeof __filename !== "undefined" ? __filename : "");
...
// ==========================================
// VERIFIED MARKET OBSERVATIONS API (STEP 4D)
// ==========================================

// Get stored market observations and summary counts
app.get("/api/pricing/market-observations", async (req, res) => {
  try {
    const { defaultMarketRepo } = await import("./src/services/pricing/marketObservationStore");
    const observations = await defaultMarketRepo.getAll();

    const now = new Date();
    const oneYearAgo = new Date(now.getTime() - 365 * 24 * 60 * 60 * 1000);
    const last365DaysCount = observations.filter((obs) => {
      const d = new Date(obs.observationDate);
      return !isNaN(d.getTime()) && d >= oneYearAgo;
    }).length;

    const distinctSourcesCount = new Set(
      observations.map((o) => (o.source || "").toLowerCase().trim()).filter(Boolean)
    ).size;

    const distinctCategoriesCount = new Set(
      observations.map((o) => (o.productCategory || "").toLowerCase().trim()).filter(Boolean)
    ).size;
    ...
    return res.json({
      ...
      observations: [...observations].reverse(), // newest first
    });
  } catch (err: any) {
    console.error("Failed to retrieve market observations:", err);
    return res.status(500).json({ success: false, error: err.message });
  }
});

// Ingest a new market observation record
app.post("/api/pricing/market-observations", async (req, res) => {
  try {
    const { defaultMarketIngestion } = await import("./src/services/pricing/marketIngestionService");
    const rawRecord = req.body;
    const result = await defaultMarketIngestion.ingestObservation(rawRecord);
    ...
    console.error("Failed to ingest market observation:", err);
    return res.status(500).json({ success: false, error: err.message });
  }
});

// Preview & validate a CSV file before importing
app.post("/api/pricing/market-observations/preview-csv", async (req, res) => {
  try {
    const { previewMarketCsv } = await import("./src/services/pricing/marketCsvImporter");
    const { defaultMarketRepo } = await import("./src/services/pricing/marketObservationStore");
    ...
    if (!csvText || typeof csvText !== "string") {
      return res.status(400).json({
        success: false,
        error: "Missing or invalid 'csvText' in request body.",
      });
    }

    const previewResult = await previewMarketCsv(csvText, defaultMarketRepo);
    ...
    console.error("Failed to preview market CSV:", err);
    return res.status(500).json({ success: false, error: err.message });
  }
});

// Batch ingest verified market observations
app.post("/api/pricing/market-observations/batch", async (req, res) => {
  try {
    const { defaultMarketIngestion } = await import("./src/services/pricing/marketIngestionService");
    ...
    const batchResult = await defaultMarketIngestion.ingestBatch(observations);
    return res.json({
      success: true,
      result: batchResult,
    });
  } catch (err: any) {
    console.error("Failed to batch ingest market observations:", err);
    return res.status(500).json({ success: false, error: err.message });
  }
});

// ==========================================
// VERIFIED ARTISAN PRODUCTS API
// ==========================================

// Get stored verified artisan products
app.get("/api/pricing/artisan-products", async (req, res) => {
  try {
    const { defaultArtisanProductRepo } = await import("./src/services/pricing/artisanProductStore");
    const products = await defaultArtisanProductRepo.getAll();
    return res.json({
      success: true,
    ...
    console.error("Failed to retrieve verified artisan products:", err);
    return res.status(500).json({ success: false, error: err.message });
  }
});

// Ingest/save verified artisan product
app.post("/api/pricing/artisan-products", async (req, res) => {
  try {
    const { defaultArtisanProductRepo } = await import("./src/services/pricing/artisanProductStore");
    const product = req.body;
    if (!product || !product.id) {
      return res.status(400).json({ success: false, error: "Missing product data or product ID." });
    }
    await defaultArtisanProductRepo.save(product);
    return res.json({ success: true });
  } catch (err: any) {
    console.error("Failed to save verified artisan product:", err);
    return res.status(500).json({ success: false, error: err.message });
  }
});
